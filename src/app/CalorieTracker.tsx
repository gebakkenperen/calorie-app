    ...
    <div className="flex flex-col items-center">
      <div className="relative" style={{ width: size, height: size }}>
        ...
        <div className="pointer-events-none absolute inset-0 flex flex-col items-center justify-center">
          <div className="text-sm text-slate-300">Vandaag</div>
      ...
      <div className="mt-4 text-center">
        {remaining >= 0 ? (
          <>
            <div className="text-sm text-slate-300">Resterend</div>
          ...
          <>
            <div className="text-sm text-slate-300">Boven doel</div>
...
function Card({
  ...
  return (
    <section className="rounded-2xl bg-surface/95 p-5 shadow-soft ring-1 ring-white/5">
...
export default function CalorieTracker() {
  ...
  const addPresetIfMissing = (name: string, kcal: number, category: Category) => {
    ...
    if (exists) return;
    setPresets((prev) => [{ id: `u_${uid()}`, name: cleanName, kcal: Math.max(0, Math.round(kcal)), category }, ...prev]);
  };
    ...
    <div className="min-h-screen bg-bg">
      <div className="mx-auto max-w-6xl px-4 py-10">
        <header className="mb-8 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
          <div>
            <div className="text-xs font-semibold tracking-widest text-slate-400">DAGELIJKSE CALORIE TRACKER</div>
            <h1 className="mt-2 text-3xl font-semibold tracking-tight text-text">Calorie App</h1>
          ...
          <div className="flex flex-wrap items-center gap-2">
            <label className="flex items-center gap-2 rounded-full bg-white/5 px-3 py-2 text-xs ring-1 ring-white/10">
              <span className="font-semibold text-slate-200">Dagelijks doel</span>
              <input
                value={goal}
                onChange={(e) => setGoal(Math.max(0, Math.round(Number(e.target.value) || 0)))}
                inputMode="numeric"
                className="w-20 bg-transparent text-right font-semibold text-text outline-none placeholder:text-slate-500"
        ...
        <div className="grid gap-6 lg:grid-cols-12">
                ...
                {(Object.keys(groupedPresets) as Category[]).map((cat) => (
                  <div key={cat}>
                    <div className="mb-2 flex items-center justify-between">
                      ...
                      <div className="text-xs text-slate-500">{groupedPresets[cat].length} items</div>
                    </div>
                    <div className="grid grid-cols-1 gap-2 sm:grid-cols-2 xl:grid-cols-3">
                      {groupedPresets[cat].map((p) => (
                        <div
                          key={p.id}
                          className="group flex items-center justify-between rounded-xl bg-white/5 p-3 ring-1 ring-white/10 transition hover:bg-white/7"
                        >
                          <div className="min-w-0">
                            <div className="truncate text-sm font-semibold text-slate-200">{p.name}</div>
                            <div className="mt-0.5 text-xs text-slate-400">{formatKcal(p.kcal)} kcal</div>
                          </div>
                          <button
                            type="button"
              ...
              <form onSubmit={onSubmitCustom} className="grid gap-3">
                <div className="grid gap-2 sm:grid-cols-3">
                  <div className="sm:col-span-2">
                    <label className="text-xs font-semibold text-slate-300">Productnaam</label>
                    <input
                      value={customName}
                      onChange={(e) => setCustomName(e.target.value)}
                      placeholder="bijv. Zelfgemaakte pasta"
                      className="mt-1 w-full rounded-xl bg-white/5 px-3 py-2 text-sm text-text ring-1 ring-white/10 outline-none placeholder:text-slate-500 focus:ring-accent/40"
                    />
                  </div>
                  <div>
                    <label className="text-xs font-semibold text-slate-300">Calorieën</label>
                    <input
                      value={customKcal}
                ...
                <button
                  type="submit"
                  className="inline-flex items-center justify-center rounded-xl bg-accent/15 px-4 py-2 text-sm font-semibold text-accent ring-1 ring-accent/30 transition hover:bg-accent/20"
                >
                  Toevoegen
                </button>

                <p className="text-xs text-slate-500">
                  Handmatige items worden toegevoegd aan je log én opgeslagen als nieuwe quick-add preset (categorie: Snacks).
                </p>
              </form>
            </Card>
          </div>
          ...
          <div className="lg:col-span-7">
            <Card
              ...
              right={<div className="text-xs text-slate-500">{log.length} items</div>}
            >
              <div className="max-h-[360px] overflow-auto pr-1">
                {log.length === 0 ? (
                  <div className="rounded-xl bg-white/5 p-4 text-sm text-slate-400 ring-1 ring-white/10">
                    Nog niets toegevoegd vandaag. Gebruik de presets of voeg een item toe via het formulier.
                  </div>
                ) : (
                  <div className="grid gap-2">
                    {log.map((it) => (
                      ...
                      >
                        <div className="min-w-0">
                          <div className="truncate text-sm font-semibold text-slate-200">{it.name}</div>
              ...
              <div className="mt-4 flex items-end justify-between rounded-xl bg-white/5 p-4 ring-1 ring-white/10">
                <div>
                  <div className="text-xs font-semibold text-slate-300">Totaal</div>
                  <div className="mt-1 text-2xl font-semibold tracking-tight text-text">{formatKcal(consumed)} kcal</div>
                </div>
                <div className="text-right text-xs text-slate-500">
                  Tip: alles blijft bewaard in je browser (localStorage).
                </div>
              </div>
            </Card>
          </div>
        ...
        <footer className="mt-10 text-xs text-slate-600">
          Palette: bg <span className="text-slate-300">#0f1117</span>, surface{" "}
          <span className="text-slate-300">#1a1d27</span>, accent{" "}
          <span className="text-slate-300">#6ee7b7</span>, danger{" "}
          <span className="text-slate-300">#f87171</span>, text{" "}
          <span className="text-slate-300">#f1f5f9</span>.
        </footer>
      </div>
    </div>
  );
